...
def get_ai_data_extraction(pdf_text):
    ...
    prompt = f"""
    分析这份 CME 交割报告，并提取 Gold, Silver, Platinum, Copper 的异动。
    要求：
    1. 识别每个品种下具体的做市商 (Firm Name) 及其 Issued (交割) 和 Stopped (接货) 数量。
    2. 为每种金属生成一句简短的异样结论。示例: "JPM 接货 500 张，暗示强支撑 | BOFA 交割 200 张"。
    3. 必须返回纯 JSON 格式，不要有 Markdown。
...
def run():
    ...
    print("🤖 正在利用 AI 分析 PDF 并生成异动结论...")
    extraction = get_ai_data_extraction(all_text)
