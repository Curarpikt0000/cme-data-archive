...
def get_ai_conclusion(pdf_text):
    """使用新版 SDK 提取异动并生成结论"""
    if not GOOGLE_API_KEY: return None
    client = genai.Client(api_key=GOOGLE_API_KEY)
    
    prompt = f"""
    分析这份 CME 交割报告文本，提取 Gold, Silver, Platinum, Copper 的异动。
    要求：
    1. 识别每个品种下具体的做市商 (Firm Name) 及其 Issued 和 Stopped 数量。
    2. 生成一句深度研判结论。示例: "JPM 接货 500 张 (Stopped)，暗示多头护盘有力；BOFA 交割 200 张"。
    3. 必须返回纯 JSON 格式，不要 Markdown：
       {{"Gold": "结论", "Silver": "结论", "Platinum": "结论", "Copper": "结论"}}
...
def run_step_3():
    ...
    print("🤖 正在利用 AI 分析 PDF 异动并分类生成结论...")
    extraction = get_ai_conclusion(all_text)
